...
/**
 * @brief 此函数用来自动计算传感器的安装高度
 * 
 * @tparam PointT 
 * @param X 
 * @param ranges 1 * linearities 评判flatness
 * @param weights 
 * @return double 
 */
template<typename PointT>
...
double PatchWork<PointT>::consensus_set_based_height_estimation(const Eigen::RowVectorXd& X,
    ...
    if(only_one_element)
        std::cout << "Only one block satisfies the flatness and uprightness requirement, which may cause some problems." << std::endl;
    // assert(!only_one_element); // tag: admit a trivial solution 如果只有一个元素，程序就会终止
    // 简单解释一下： X是一个1*N的向量，对当前帧以圆柱坐标系划分20块，满足upright和flat的经验阈值的条件就会把那一块之内的点云的z方向均值，存在X的dim2中。那么如果只有一块满足了这个阈值条件，就只有一个elem，程序会终止
    ...
    cout << "min_idx: " << min_idx << "\nsize of x_hat: " << x_hat.size() << endl;
    if (min_idx == 0 && x_hat.size() == 0)
        return sensor_height_; // 都为0时，直接把外部输入的sensor height输出

    double estimate_temp = x_hat(min_idx); // tag: debug结论：当x_hat和min_idx都为0时，会终止程序，报错可看后面博客 https://blog.csdn.net/Rensandao/article/details/83539086
...
void PatchWork<PointT>::estimate_ground(
    ...
    if (initialized_ && ATAT_ON_) {
        estimate_sensor_height(cloud_in); // tag: Assertion `index >= 0 && index < size()' failed. 错误发生在这里
        initialized_ = false;
    }
